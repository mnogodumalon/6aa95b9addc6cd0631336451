...
### `behandler` — Behandler (type `Behandler`, one record: `BehandlerEntry`)
...
- `kuerzel` · text · Kürzel · required! · maxLength 4000
- `fachgebiete` · multilookup · Fachgebiete · options: manuelle_therapie | krankengymnastik | lymphdrainage | massage | sportphysio
- `arbeitstage` · multilookup · Arbeitstage · options: montag | dienstag | mittwoch | donnerstag | freitag
- `status` · lookup · Status · required! · options: aktiv | urlaub | krank
- `email` · email · E-Mail
Input shapes: choice `status` (ChoiceGroup)

### `patienten` — Patienten (type `Patienten`, one record: `PatientenEntry`)
Display fields: `vorname`, `nachname`
- `vorname` · text · Vorname · required! · maxLength 4000
- `nachname` · text · Nachname · required! · maxLength 4000
...
- `krankenkasse` · text · Krankenkasse · maxLength 4000
- `versichertennummer` · text · Versichertennummer · maxLength 4000
- `versicherungsart` · lookup · Versicherungsart · required! · options: gesetzlich | privat | selbstzahler
- `hausarzt` · text · Hausarzt · maxLength 4000
- `notizen` · textarea · Notizen (vertraulich)
Input shapes: choice `versicherungsart` (ChoiceGroup)

### `leistungen` — Leistungen (type `Leistungen`, one record: `LeistungenEntry`)
Display fields: `bezeichnung`
- `bezeichnung` · text · Bezeichnung · required! · maxLength 4000
- `heilmittelkuerzel` · text · Heilmittelkürzel · required! · maxLength 4000
- `dauer_minuten` · number · Dauer (Minuten) · required!
- `preis_kasse` · number · Preis Kasse (€) · currency
- `preis_privat` · number · Preis Privat (€) · currency

### `rezepte` — Rezepte (type `Rezepte`, one record: `RezepteEntry`)
Display fields: `arztpraxis`
- `patient` · record · Patient · required! · → `patienten`
- `ausstellungsdatum` · date · Ausstellungsdatum · required!
- `arztpraxis` · text · Verordnende Arztpraxis · required! · maxLength 4000
- `diagnose` · textarea · Diagnose
- `leistung` · record · Leistung · required! · → `leistungen`
- `anzahl_einheiten` · number · Anzahl verordnete Einheiten · required!
- `frequenz_pro_woche` · number · Frequenz pro Woche
- `gueltig_bis` · date · Gültig bis
- `status` · lookup · Status · required! · options: offen | in_behandlung | abgeschlossen | abgelaufen
Input shapes: choice `status` (ChoiceGroup)

### `termine` — Termine (type `Termine`, one record: `TermineEntry`)
Display fields: `bemerkung`
- `patient` · record · Patient · required! · → `patienten`
- `behandler` · record · Behandler · required! · → `behandler`
- `rezept` · record · Rezept (optional) · → `rezepte`
- `beginn` · datetime · Datum und Beginn · required!
- `ende` · datetime · Ende · required!
- `raum` · lookup · Raum · required! · options: raum_1 | raum_2 | raum_3 | geraeteraum
- `status` · lookup · Status · required! · options: geplant | bestaetigt | erschienen | nicht_erschienen | abgesagt
- `bemerkung` · textarea · Bemerkung
Input shapes: range `beginn`+`ende` (AvailabilityRangePicker); choice `raum` (ChoiceGroup); choice `status` (ChoiceGroup)

### `behandlungen` — Behandlungen (type `Behandlungen`, one record: `BehandlungenEntry`)
Display fields: `befund`
- `termin` · record · Termin · required! · → `termine`
- `durchgefuehrte_leistung` · record · Durchgeführte Leistung · required! · → `leistungen`
- `dauer_tatsaechlich` · number · Tatsächliche Dauer (Minuten)
- `befund` · textarea · Befund (vertraulich)
- `massnahmen` · textarea · Maßnahmen
- `verlauf` · textarea · Verlauf
- `naechste_schritte` · textarea · Nächste Schritte
