/**
 * PatientenDialog — pre-generated create/edit dialog for Patienten.
 *
 ...
 * PatientenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Patienten['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<PatientenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Patienten, LookupValue } from '@/types/app';
...
/** Widened prefill type for PatientenDialog.defaultValues — see file header. */
export type PatientenDialogDefaults = Omit<Patienten['fields'], 'versicherungsart'> & {
    versicherungsart?: LookupValue | string;
  };
...
interface PatientenDialogProps {
  ...
  defaultValues?: PatientenDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  versicherungsart: LOOKUP_OPTIONS['patienten']?.['versicherungsart'] ?? [],
};
...
export function PatientenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: PatientenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'geburtsdatum', 'versicherungsart'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Patienten['fields']>);
      setPreview(null);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('patienten') })
    : t('new_entity', { entity: appLabel('patienten') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        ...
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('patienten', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('patienten', 'nachname')}</p>
        )}
      </div>
    ),
    'geburtsdatum': (
      <div key="geburtsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="geburtsdatum"
          placeholder="Wann wurde der Patient geboren?"
          mode="date"
        ...
        {showErrors && !fields.geburtsdatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('patienten', 'geburtsdatum')}</p>
        )}
      </div>
    ),
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('patienten', 'telefon')}</Label>
        <Input
          id="telefon"
          type="tel"
          inputMode="tel"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('patienten', 'email')}</Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
          placeholder="z. B. patient@example.de"
          value={fields.email ?? ''}
    ...
    'krankenkasse': (
      <div key="krankenkasse" className="space-y-1.5">
    ...
    'versichertennummer': (
      <div key="versichertennummer" className="space-y-1.5">
    ...
    'versicherungsart': (
      <div key="versicherungsart" className="space-y-1.5">
        <Label htmlFor="versicherungsart">{fieldLabel('patienten', 'versicherungsart')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.versicherungsart) === 'gesetzlich'}
            onClick={() => setFields(f => ({ ...f, versicherungsart: (lookupKey(f.versicherungsart) === 'gesetzlich' ? undefined : 'gesetzlich') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.versicherungsart) === 'gesetzlich'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('patienten', 'versicherungsart', 'gesetzlich') ?? 'Gesetzlich'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.versicherungsart) === 'privat'}
            onClick={() => setFields(f => ({ ...f, versicherungsart: (lookupKey(f.versicherungsart) === 'privat' ? undefined : 'privat') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.versicherungsart) === 'privat'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('patienten', 'versicherungsart', 'privat') ?? 'Privat'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.versicherungsart) === 'selbstzahler'}
            onClick={() => setFields(f => ({ ...f, versicherungsart: (lookupKey(f.versicherungsart) === 'selbstzahler' ? undefined : 'selbstzahler') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.versicherungsart) === 'selbstzahler'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('patienten', 'versicherungsart', 'selbstzahler') ?? 'Selbstzahler'}
          </button>
        </div>
        {showErrors && !fields.versicherungsart && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('patienten', 'versicherungsart')}</p>
        )}
      </div>
    ),
    'hausarzt': (
      <div key="hausarzt" className="space-y-1.5">
        <Label htmlFor="hausarzt">{fieldLabel('patienten', 'hausarzt')}</Label>
        <Input
          id="hausarzt"
          placeholder="z. B. Dr. Hans Weber"
