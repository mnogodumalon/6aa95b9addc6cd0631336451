/**
 * LeistungenDialog — pre-generated create/edit dialog for Leistungen.
 *
 ...
 * LeistungenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Leistungen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<LeistungenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Leistungen } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Leistungen';
...
/** Widened prefill type for LeistungenDialog.defaultValues — see file header. */
export type LeistungenDialogDefaults = Leistungen['fields'];

interface LeistungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Leistungen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: LeistungenDialogDefaults;
...
export function LeistungenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: LeistungenDialogProps) {
  const [fields, setFields] = useState<Partial<Leistungen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['bezeichnung', 'heilmittelkuerzel', 'dauer_minuten'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Leistungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'leistungen');
      await onSubmit(clean as Leistungen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "bezeichnung": string | null, // Bezeichnung\n  "heilmittelkuerzel": string | null, // Heilmittelkürzel\n  "dauer_minuten": number | null, // Dauer (Minuten)\n  "preis_kasse": number | null, // Preis Kasse (€)\n  "preis_privat": number | null, // Preis Privat (€)\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Leistungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('leistungen') })
    : t('new_entity', { entity: appLabel('leistungen') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        ...
        {showErrors && !fields.bezeichnung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('leistungen', 'bezeichnung')}</p>
        )}
      </div>
    ),
    'heilmittelkuerzel': (
      <div key="heilmittelkuerzel" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.heilmittelkuerzel && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('leistungen', 'heilmittelkuerzel')}</p>
        )}
      </div>
    ),
    'dauer_minuten': (
      <div key="dauer_minuten" className="space-y-1.5">
        <Label htmlFor="dauer_minuten">{fieldLabel('leistungen', 'dauer_minuten')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="dauer_minuten"
          type="number"
          inputMode="decimal"
        ...
        />
        {showErrors && !fields.dauer_minuten && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('leistungen', 'dauer_minuten')}</p>
        )}
      </div>
    ),
    'preis_kasse': (
      <div key="preis_kasse" className="space-y-1.5">
        <Label htmlFor="preis_kasse">{fieldLabel('leistungen', 'preis_kasse')}</Label>
        <Input
          id="preis_kasse"
          type="number"
          inputMode="decimal"
    ...
    'preis_privat': (
      <div key="preis_privat" className="space-y-1.5">
        <Label htmlFor="preis_privat">{fieldLabel('leistungen', 'preis_privat')}</Label>
        <Input
          id="preis_privat"
          type="number"
          inputMode="decimal"
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Bezeichnung", "heilmittelkuerzel": "Heilmittelkürzel", "dauer_minuten": "Dauer (Minuten)", "preis_kasse": "Preis Kasse (€)", "preis_privat": "Preis Privat (€)"};
  const CURRENCY_KEYS = new Set<string>(["preis_kasse", "preis_privat"]);
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.LEISTUNGEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
